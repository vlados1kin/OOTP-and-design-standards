...
{
    private static readonly PluginService Plugin = new();
    
    [HttpPost]
    [Route("convert")]
    public IActionResult JsonToXml(Parameters parameters)
    {
        string extension = Path.GetExtension(parameters.Path);
        IConvertor convertor;
        switch (extension)
        {
            case ".xml":
                if (FigureService.TransformatorPlugin != null)
                {
                    convertor = new ConvertorAdapter(FigureService.TransformatorPlugin);
                    convertor.FromXmlToJson(parameters.Path);
                }
                break;
            case ".json":
                convertor = new Convertor();
                convertor.FromJsonToXml(parameters.Path);
                break;
        }
        return NoContent();
    }
    ...
    public ActionResult<string> CheckSign(string sign)
    {
        if (FigureService.Dll.ContainsKey(sign))
        {
            ...
            Signature? signature = Plugin.GetSignature(plugin);
    ...
    public async Task<IActionResult> UploadFile(IFormFile? file)
    {
        if (file != null && file.Length > 0)
        {
            ...
            Assembly pluginAssembly = Plugin.LoadPlugin(filePath);
            ...
            IEncryptor? encryptor = Plugin.GetEncryptor(pluginAssembly);
            if (encryptor != null)
            {
                FigureService.EncryptPlugin = encryptor;
            }

            IZipper? zipper = Plugin.GetZipper(pluginAssembly);
            if (zipper != null)
            {
                FigureService.ZipperPlugin = zipper;
            }
            
            ITransformator? transformator = Plugin.GetTransformator(pluginAssembly);
            if (transformator != null)
            {
                FigureService.TransformatorPlugin = transformator;
            }
            return Ok();
        }
        else
